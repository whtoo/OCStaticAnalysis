...
public class OCInterpreter {
    ...
    func eval(variable: OCVar) -> OCValue {
        guard let value = scopes[variable.name] else {
            fatalError("Error: eval var error")
        }
    ...
    func eval(binOp: OCBinOp) -> OCValue {
        guard case let .number(leftResult) = eval(node: binOp.left), case let
                .number(rightResult) = eval(node: binOp.right) else {
